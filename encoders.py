...
class GraphConv(nn.Module):
    ...
    def forward(self, x, adj):
        ...
        if self.add_self:
            y += x #(adj+I)x=adj*x+I*x
        y = torch.matmul(y,self.weight)#LXW
...
class GcnEncoderGraph(nn.Module):
    def __init__(self, input_dim, hidden_dim, embedding_dim, label_dim, num_layers,
            pred_hidden_dims=[], concat=True, bn=True, dropout=0.0, args=None,assign_input_dim=0,res_dim=0):
        ...
        if concat:
            self.pred_input_dim = hidden_dim * (num_layers - 1) + embedding_dim #+ assign_input_dim
        else:
            self.pred_input_dim = embedding_dim

        if args is not None and args.res:
            self.pred_input_dim += res_dim
    ...
    def apply_bn(self, x):
        ''' Batch normalization of 3D tensor x    32*80*6  32*6
        '''
    ...
    def gcn_forward(self, x, adj, conv_first, conv_block, conv_last, embedding_mask=None):
        ...
        x_tensor = torch.cat(x_all, dim=2) #20*500*64,20*500*64,....20*500*64
        if embedding_mask is not None:
            x_tensor = x_tensor * embedding_mask #200*500*192
        return x_tensor
    ...
    def forward(self, x, adj,batch_num_nodes=None, **kwargs):
        ...
        if self.num_aggs == 2:
            ...
            out_all.append(out)
        # res_x = assign_x
        # out_all.append(res_x.reshape(res_x.shape[0],-1))
        if self.concat:
...
class SoftPoolingGcnEncoder(GcnEncoderGraph):
    def __init__(self, max_num_nodes, input_dim, hidden_dim, embedding_dim, label_dim, num_layers,
            ...
            assign_input_dim=-1, args=None,res_dim=0):
        '''
        Args:
        ...
        super(SoftPoolingGcnEncoder, self).__init__(input_dim, hidden_dim, embedding_dim, label_dim,
                num_layers, pred_hidden_dims=pred_hidden_dims, concat=concat, args=args,assign_input_dim=assign_hidden_dim)
        add_self = not concat
        ...
        res_pred_input_dim = self.pred_input_dim * (num_pooling+1)
        if args is not None and args.res:
            res_pred_input_dim+=res_dim
        self.pred_model = self.build_pred_layers(res_pred_input_dim, pred_hidden_dims,
                label_dim, num_aggs=self.num_aggs)
    ...
    def forward(self, x, adj, batch_num_nodes, **kwargs):
        ...
        out, _ = torch.max(embedding_tensor, dim=1) #20*192
        out_all.append(out)
        if self.num_aggs == 2:
        ...
        for i in range(self.num_pooling):
            ...
            self.assign_tensor = self.gcn_forward(x_a, adj, 
                    ...
                    embedding_mask) #200*500*(64*2+500*0.1)
            # [batch_size x num_nodes x next_lvl_num_nodes]
            self.assign_tensor = nn.Softmax(dim=-1)(self.assign_pred_modules[i](self.assign_tensor)) # 64*2+50--->50
            if embedding_mask is not None:
                self.assign_tensor = self.assign_tensor * embedding_mask

            # update pooled features and adj matrix
            x = torch.matmul(torch.transpose(self.assign_tensor, 1, 2), embedding_tensor)#(20*50*500)matmul(20*500*192)---->20*50*192
            adj = torch.transpose(self.assign_tensor, 1, 2) @ adj @ self.assign_tensor  #(20*50*500)@(20*500*500)@(20*500*50)--->20*50*50
            x_a = x
        
            embedding_tensor = self.gcn_forward(x, adj, 
                    self.conv_first_after_pool[i], self.conv_block_after_pool[i],
                    self.conv_last_after_pool[i])  # 20*50*192
        ...
        if res_x is not None and self.args.res:
            out_all.append(res_x)
        if self.concat:
            output = torch.cat(out_all, dim=1) # out_all.shape = 1+num_pooling  20*((1+num_pooling)*192)
        else:
            output = out
        ypred = self.pred_model(output)
        return ypred
